...
export function DestinationModal({ destination, onClose }: DestinationModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative bg-card border border-border rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden animate-in zoom-in-95 fade-in duration-300">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col md:flex-row max-h-[90vh]">
          ...
          <div className="flex-1 p-6 md:p-8 overflow-y-auto">
            <div className="mb-6">
              <span className="text-primary text-sm tracking-widest uppercase">{destination.era}</span>
              <h2 className="text-3xl md:text-4xl font-serif text-foreground mt-2">{destination.title}</h2>
            </div>
            ...
            <div className="flex flex-wrap gap-4 mb-6">
              ...
              <div className={`flex items-center gap-2 ${dangerColors[destination.danger]}`}>
                <AlertTriangle className="w-5 h-5" />
                <span className="capitalize">Risque {destination.danger === 'low' ? 'Faible' : destination.danger === 'medium' ? 'Modéré' : 'Élevé'}</span>
              </div>
            </div>
            ...
            <div className="flex gap-4">
              ...
              <Button
                variant="outline"
                className="border-border text-foreground hover:bg-secondary rounded-full bg-transparent"
                onClick={onClose}
              >
                Retour
              </Button>
